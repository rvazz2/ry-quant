...
import { Skull, TrendingUp, TrendingDown, RefreshCcw } from 'lucide-react';
...
const InverseCramer = () => {
    ...
    useEffect(() => {
        const fetchCramer = async () => {
            try {
                ...
                console.error("Failed to fetch Inverse Cramer data", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) return (
        <div className="glass-panel p-6 h-64 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-slate-500 text-sm animate-pulse">Tracking contrarian signals...</p>
        </div>
    );
    ...
    if (!data) return (
        <div className="glass-panel p-6 h-64 flex flex-col items-center justify-center gap-4 border-l-4 border-rose-500/50">
            ...
            <p className="text-slate-400">Unable to load Inverse Cramer data.</p>
        </div>
    );
        ...
        <div className="glass-panel p-6 border-l-4 border-l-purple-600 space-y-6">
            <div className="flex justify-between items-start">
                <div>
                    <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
                        ...
                        The Inverse Cramer Tracker
                    </h2>
                    <p className="text-slate-400 text-sm mt-1">
                        Tracking the performance of doing exactly the opposite of TV pundits.
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                ...
                <div className="p-4 bg-purple-900/20 rounded-xl border border-purple-500/50 text-center">
                    <div className="text-xs text-purple-400 uppercase font-bold">Inverse Cramer</div>
                    <div className="text-2xl font-black text-white">{data.summary.InverseCramer_Return}</div>
                </div>
                <div className="p-4 bg-slate-900 rounded-xl border border-slate-800 text-center">
                    <div className="text-xs text-slate-500 uppercase font-bold">Alpha Generated</div>
            ...
            <div className="h-64 mt-4">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data.series}>
                        ...
                        <Line type="monotone" dataKey="InverseCramer" stroke="#a855f7" strokeWidth={3} dot={false} activeDot={{ r: 6 }} />
                    </LineChart>
                </ResponsiveContainer>
            </div>

            <div className="text-xs text-slate-500 italic text-center">
                * Hypothetical performance based on contra-momentum algorithm. Not financial advice.
            </div>
        </div>
    );
};
